...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        w_reg = np.random.randn(X.shape[1]) / np.sqrt(X.shape[1])

        if self.penalty_ == "none":
            module = LogisticModule(w_reg)
        elif self.penalty_ == "l1":
            module = RegularizedModule(LogisticModule(w_reg), L1(),
                                              self.lam_, w_reg,
                                              self.include_intercept_)
        else:
            module = RegularizedModule(LogisticModule(w_reg), L2(),
                                              self.lam_, w_reg,
                                              self.include_intercept_)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.include_intercept_:
            X = np.hstack((np.ones((X.shape[0], 1)), X))
        xw = np.dot(X, self.coefs_)
        return np.exp(xw) / (np.exp(xw) + 1)
